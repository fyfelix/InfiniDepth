...
Environment overrides:
  DATASET_PATH          TransCG JSONL path. Default: data/TransCG/transcg/dc_testset_d435_005ratio.jsonl
  OUTPUT_DIR            Prediction/evaluation output directory. Default: <checkpoint_dir>/transcg_<jsonl_stub>_<checkpoint_stub>_data_d435
...
TransCG is fixed to raw-type=d435 and uses uint16 PNG depth in millimeters.
EOF
}
...
dataset_path="${DATASET_PATH:-data/TransCG/transcg/dc_testset_d435_005ratio.jsonl}"
...
output_dir="${OUTPUT_DIR:-${model_dir}/transcg_${dataset_stub}_${model_stub}_data_${camera_type}}"
...
echo "evaluating the model on TransCG"
